...
public class RewardsLastHitViewGUI extends GUI{

    String dragon;
    GUI guiAnterior;

    public RewardsLastHitViewGUI(Core plugin, Player player, String dragon, GUI guiAnterior) {
        super(plugin, player);
        this.dragon = dragon;
    ...
    public void buildAndOpenGUI() {
        ...
        String lootTable = dragonConfig.getString(DragonsConfig.RECOMPENSA_ULTIMO_GOLPE);

        chestMenu = new ChestMenu(ChatColor.DARK_GRAY + "Loot último golpe", 54, event -> {
            ...
            switch (event.getName()){
                ...
                case "&eElegir otra LootTable":
                    event.setWillDestroy(true);
                    delayEntreMenues(new RewardsLastHitChangeGUI(plugin,player,dragon,this));
                    return;
                case "&9Volver":
                    event.setWillDestroy(true);
