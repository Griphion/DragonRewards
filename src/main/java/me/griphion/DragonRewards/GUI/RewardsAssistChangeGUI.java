...
public class RewardsAssistChangeGUI extends GUI{
    GUI guiAnterior;
    String dragon;
    public RewardsAssistChangeGUI(Core plugin, Player player, String dragon, GUI guiAnterior) {
        super(plugin, player);
        this.dragon = dragon;
    ...
    public void buildAndOpenGUI() {
        ...
        chestMenu = new ChestMenu(ChatColor.DARK_GRAY + "Elija otra LootTable para los Asist", 54, event -> {
            ...
            if (event.getPosition() < 36) {
                ...
                if (laLootTableNoExiste(lootTable, player)) {
                    ...
                    dragonConfig.getConfig().set(DragonsConfig.RECOMPENSA_ASISTENCIA,lootTable);
        ...
        agregarLootTables(chestMenu, Arrays.asList("",ChatColor.GRAY + "Click izquierdo para asignar esta LootTable a las Asistencias"));
